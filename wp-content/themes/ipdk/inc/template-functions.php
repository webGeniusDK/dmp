 ...
 * @return array
 */
function ipdk_body_classes( $classes ) {
...
add_filter( 'body_class', 'ipdk_body_classes' );

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function ipdk_pingback_header() {
	if ( is_singular() && pings_open() ) {
		printf( '<link rel="pingback" href="%s">', esc_url( get_bloginfo( 'pingback_url' ) ) );
	}
}
add_action( 'wp_head', 'ipdk_pingback_header' );
...
function insertSVG( $svg_name, $viewbox = NULL ) {
	return sprintf('<svg %s><use xlink:href="' . get_stylesheet_directory_uri() . '/icons/ipdk-icon-sprite.svg#' . $svg_name . '" href="' . get_stylesheet_directory_uri() . '/icons/ipdk-icon-sprite.svg#' . $svg_name . '"></use></svg>', $viewbox ? 'viewbox="' . $viewbox . '"' : '');
}
